//! yr.no Locationforecast 2.0 wire format, fetch, parsing, symbol mapping.
//!
//! Split from `weather_live.rs` (file-length limit): everything that touches
//! the MET Norway payload lives here; location presets, telemetry model, and
//! cache orchestration stay in `weather_live.rs`.

use serde::Deserialize;
use std::process::Command;
use std::time::{SystemTime, UNIX_EPOCH};

use super::weather_live::{
    location_by_index, CurrentWeather, DayForecast, WeatherLocation, WeatherTelemetry,
};
...
pub fn fetch_locationforecast(loc: &WeatherLocation) -> Option<String> {
    let url = format!(
...
pub fn civil_date(days: i64) -> (i64, u64, u64) {
    let z = days + 719_468;
...
/// Parse the yr.no compact payload into current conditions + 10-day forecast.
pub fn parse_forecast(
    ...
    // ---- Day forecast: group by local date, today first ----
    ...
    for (day, list) in days.iter().take(11) {
        if *day < today || t.days.len() >= 10 {
            continue;
        }
